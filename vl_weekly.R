...
#set working directory
  #setwd('C:\\power bi')
  setwd('C:\\Users\\MARYLAND\\OneDrive - University of Maryland School of Medicine\\HIS\\weekly data reporting tool')
  
#kobotoolbox settings

# Define the API endpoint URL
  endpoint_url <- paste0("https://kc.kobotoolbox.org/api/v1/data/1239833")

# Get a token
  token <- kobo_token(username = "konli90", password = "Malawi-MGIC#2022", 
...
# remove the other dataframes
  rm(list=c('response', 'submissions_df', 'response_content', 'endpoint_url', 'token', 'date_range'))
...
# replace hub_received_wide dataframe NA with 0
  hub_received_wide <- hub_received_wide |>
    mutate_at(c('Area25', 'Blantyre','Bwaila', 'Chikwawa', 'Chiradzulu','Chitipa','Dedza', 'Dowa', 'Karonga',
                'Kasungu', 'Machinga', 'Mangochi', 'Mchinji', 'Mulanje', 'Mwanza', 'Mzuzu', 'Neno', 'Nkhatabay',
                'Nkhotakota','Ntchisi', 'Rumphi', 'Ntcheu'), ~replace_na(.,0))
  
# sort dataframe by week
  hub_received_wide <- hub_received_wide %>% arrange(desc(yearweek))
  
# select top 7 rows
  hub_received_wide <- head(hub_received_wide, 7)
...
# select top 7 rows
  ml_total_tests_wide <- head(ml_total_tests_wide, 7) 
